...
{container stylesheet="profile_music_album.style"}
	...
	{else}
		...
		{if $first.music_id}
			...
			{else}
				<a href="{$first.music_page}">{$first.title|out_format|smile|censor:"music":true}</a><br />
		...
		{if $first_top.music_id}
			...
			{else}
				<a href="{$first_top.music_page}">{$first_top.title|out_format|smile|censor:"music":true}</a><br />
