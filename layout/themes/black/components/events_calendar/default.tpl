        ...
        {foreach from=$calendar_array item='week'}
        	<tr>
            	{foreach from=$week item='day'}
               	<td{if $day} class="{if $day.current}current{elseif $day.active}active{else}day{/if}{if $day.link} link{/if}"{/if}>{if $day}{if $day.link}<a href="{$day.url}">{$day.date}</a>{else}{$day.date}{/if}{/if}</td>
                {/foreach}
            </tr>
        {/foreach}
    </table>
    <div style="float:left;width:48%;"><a href="{$prev_array.url}">&laquo; {$prev_array.label}</a></div><div style="width:48%;float:right;text-align:right;"><a href="{$next_array.url}">{$next_array.label} &raquo;</a></div><br class="clr" />
    {/block}
{/container}
