...
{container stylesheet="style.style"}
    ...
    {/block_cap}    
    {block}
    <div class="index_calendar_title">{$index_label}</div>
	
{if !empty($calendar)}
    	<div {id="calendar_cont"}{if !empty($cal_none)} style="display:none;"{/if}>
            <table class="event_calendar" cellspacing="1">
                ...
                {foreach from=$calendar_array item='week'}
                    <tr>
                        ...
                        <td{if $day} class="{if $day.current}current {/if}{if $day.active}active{else}day{/if}{if $day.link} link{/if}"{/if}>{if $day}{if $day.link}<a href="{$day.url}"><span>{$day.date}</span></a>{else}<span>{$day.date}</span>{/if}{/if}</td>
                        {/foreach}
                    </tr>
                {/foreach}
            </table>
...
{/if}
    ...
    {if !$no_events}<div class="clearfix"><div class="block_toolbar"><a href="{$events_url}">{text %.event.view_all}</a></div></div>{/if}
    {/block}
{/container}
