...
{container stylesheet="style.style"}
    ...
    {/block_cap}
    {if !$no_events}<div class="block_toolbar"><div class="block_toolbar_r"><div class="block_toolbar_c clearfix"><a href="{$events_url}">{text %.event.view_all}</a></div></div></div>{/if}
    {block}
    <div style="text-align:center;font-weight:bold;padding-bottom:4px;">{$index_label}</div>
