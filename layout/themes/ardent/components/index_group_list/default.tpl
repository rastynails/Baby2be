				...
				{foreach from=$list.groups item='group'}
					<li class="item">
						<div class="list_thumb">
							<a href="{document_url doc_key='group' group_id=$group.group_id}"><img title="{$group.title|censor:'blog':true}" src="{$group.img}" width="90" /></a>
						</div>
						<div class="list_block" style="overflow: hidden">
							<div class="list_info">
								<div class="item_title"><a href="{document_url doc_key='group' group_id=$group.group_id}">{$group.title|truncate:60|censor:'blog':true}</a></div>
								{text %by} {$group.username}, {$group.creation_stamp|spec_date}
							</div>
							<div class="list_content">{$group.description|smile|out_format|truncate:160|censor:'blog'}</div>
						</div>
						<div class="clr" />
					</li>
				{/foreach}
			</ul>
			<div class="clearfix"><div class="block_toolbar"><a href="{document_url doc_key='groups'}">{text %view_all}</a></div></div>
		</div>
		{/foreach}
	</div>
	{/if}
