...
{container stylesheet="group_list.style"}
	...
	{else}
		{foreach from=$groups item='group' name=gr}
			...
			{block title=$group.title|out_format|truncate:40|censor:'forum':true}
				...
				<div class="group_info">
					<div class="group_descr">{$group.description|smile|out_format|censor:'forum'}</div>
					<div class="group_stat small">
						{text %.label.by} 
						{if $group.profile_id}
							<a href="{document_url doc_key='profile' profile_id=$group.profile_id}">{$group.username}</a>
						{else}
