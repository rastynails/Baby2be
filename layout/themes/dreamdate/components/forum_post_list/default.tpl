	   ...
	   <td>
		{foreach from=$posts item=post}
		 <table width="100%" class="post_table">
			<tr class="{cycle values='trc_1,trc_2'}" {id=post_`$post.forum_post_id`}>
				<td class="forum_thumb">
				    <div class="clearfix">
                        <div class="float_left">
                            {profile_thumb profile_id=$post.profile_id size=85}    
                        </div>
                        <div class="float_left">
                                {if !$post.is_deleted}
                                    <div class="username">
	                                    <a href="{document_url doc_key='profile_view' profile_id=$post.profile_id}">
	                                        {$post.username}
	                                    </a>
	                                </div>
	                                <div class="pinfo">
	                                   {text %.profile_fields.value.sex_`$post.profile_sex`}
	                                   {if $post.profile_age}{$post.profile_age} {text %years_old}{/if}
	                                </div>
