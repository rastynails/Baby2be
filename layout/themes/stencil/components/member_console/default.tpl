...
{container class="member_console" stylesheet="member_console.style"}
	...
	{block class="member_block" title=$profile.username|truncate:18:"...":true}
		<div style="display:none"><input id="delete_t_text" value="{text %delete_thumb_msg}" /></div>
		{if $profile.hasThumb}
			<div class="delete_thumb_wrap"><a href="javascript://" class="delete_thumb">{text %delete_thumb}</a></div>
		{/if}
		<div class="center thumb_wrap">
			<div class="thumb_container">
				{profile_thumb profile_id=$profile.id href=$profile.photo_upload_url size=60 sexLine=false}
			</div>
