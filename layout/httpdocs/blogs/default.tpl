...
{canvas}
    ...
    <div class="float_half_left wider">
        {block title=%.blogs.blogs_page_cap_label}
            {if $no_posts}
                    ...
                    {foreach from=$bp_list item='blog_entry'}
                        <li class="item">
                            <div class="list_thumb">
                                <a href="{$blog_entry.temp.blog_post_url}">{profile_thumb profile_id=$blog_entry.dto->getProfile_id() size='60' href=false}</a>
                            </div>
                            <div class="list_block">
                                ...
                                <div class="list_content">
                                    {$blog_entry.temp.preview_text|smile|out_format:'blog'|censor:'blog'}
                                </div>
                            </div>
                            <br clear="all" />
                        </li>
    ...
    <div class="float_half_right narrower">
        {block title=%.blogs.stat_cap_label}
            {text %.blogs.stat_label_blogs}: <span class="highlight">{$b_count}</span><br />
            {text %.blogs.stat_label_posts}: <span class="highlight">{$bp_count}</span>
        {/block}
