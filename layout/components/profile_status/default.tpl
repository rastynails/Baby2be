...
{capture name=set_status_link}
	<div {id="profile_status_toggle_cont"}></div>
{/capture}
	
{block title=%profile_status_header toolbar=$smarty.capture.set_status_link}
<table width="100%">
	<tbody>
		<tr>
		...
		<tr>
			<td colspan="2">
				...
				{form UserStatus}
					...
					<div class="float_left">
						{input name="status"}
						<span class="small">{text %constraint_profile_status_length length=40}</span>
					</div>
					<div class="float_left">{text_formatter for='status' controls='emoticon' entity="profile_status"}</div>
					<div class="float_left">{button action="update"}</div>
				{/form}
				</div>
			</td>
		</tr>
