		...
		<div class="float_half_left wider">
			...
			{capture name=toolbar}
				{if $permission.avaliable}
					<a href="{$photo.fullsize_src}" {id="fullsize_btn"} target="_blank">{text %full_s_btn}</a>
				{/if}
			{/capture}
						
			{block id="screen" title=$photo.title|censor:"photo":true toolbar=$smarty.capture.toolbar}
				...
				{if $permission.avaliable}
					...
					<ul {id="carousel"} class="jcarousel-skin" style="display:none">
						...
						<li>
							<a href="{$item.fullsize_url}"><img {if $photo.id==$item.id} class="active" {/if} height="50" width="50" src="{$item.thumb_src}" title="{$item.title|censor:'photo':true}" /></a>
						</li>
						{/foreach}
					</ul>
				{/if}
