{* component Invite Friends *}
 ...
 {container stylesheet="invite_friends.style" class="invite_friends_cont"}
	...
	<div class="invite_form_cont">
	 	{block}
	 	{block_cap title=%block_label}{component ContactImporterButtons}{/block_cap}
	 		{*if $fb_invite_configured}
	 		<center>
 				<button id="fbib" onclick="Fb_Invite();" >{text %fb_invite_txt}</button>
 				<hr />
 			</center>
		 	...
		 	{form InviteFriends}
				<p>
					{label for='email_addr'}{if $contactGrabber}&nbsp;<a href="javascript://" {id="open_box_btn"}>{text %import.open_btn}</a>{/if}
