			...
			{block}
			  ...
			  <!-- div style="position: relative; z-index: 98" -->
				...
				<div class="profile_list_info">
					...
					<br /><br />
					<span class="small">
						{$item.general_description|wordwrap:62:"<br />":true|strip_tags|nl2br|censor:'profile'}
					</span>
					{if $item.note}
					<br /><br />
					<span class="small">{text %.components.profile_notes.note_about} {$item.profile_label}:<br />
                        {$item.note}
                    </span>
					{/if}
				</div>
