...
{container stylesheet="forum_group_list.style"}
	...
	{block title=$group.name class="block_null"}
		<table class="form">
		<thead>
			<tr>
		...
		{foreach from=$group.forums item=forum}
			<tr class="list_item">
				<td class="status"></td>
				<td><a href="{document_url doc_key='forum' forum_id=$forum.forum_id}" class="bold">{$forum.name|censor:'forum'}</a><br />
					<span class="small">{$forum.description|censor:'forum'}
				</span></td>
