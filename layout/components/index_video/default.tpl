...
{container stylesheet="index_video.style"}
	...
	<div class="video_cont" {id="video_cont"}>
		...
		<div class="video_list" {id="video_`$list_name`"} style="display: {if $list.is_active}block;{else}none;{/if}">
			...
			{else}
			<div class="block_toolbar"><a href="{$video[$list_name].view_more}">{text %more}</a></div>
			<div class="video_title"><a href="{$list.player.video_page}">{$list.player.title|out_format|censor:"video":true}</a></div>
