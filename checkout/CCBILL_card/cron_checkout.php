...
if ( curl_errno($ch) )
{
	...
	CCBILLcardSaveInLogFile($logString, 'errors.log');
	return;
}
else
	$_rebill_arr = parseCCBILLcardDatalinkResponse( $pp_FORMAT, $_string );
...
CCBILLcardSaveInLogFile( $logString, 'transactions.log' );
...
foreach ( $_rebill_arr as $_rebill )
{
	...
	switch ( strtoupper($_rebill['type']) )
	{
		...
		case 'REBILL':
			...
			CCBILLcardSaveInLogFile( "Custom = ".$custom." | Subscription ID = ".$_rebill['subscription_id']." | Transaction ID = ".$trans_order." | Transaction result: ".$res, 'transactions.log' );
			if ( $res )
			{
				app_Finance::LogPayment( 'Registered.', __FILE__, __LINE__ );
				$__CUSTOM = $custom;
		...
		case 'EXPIRE':
            ...
            $query = SK_MySQL::placeholder("SELECT `m`.`membership_id`, `s`.* 
                ...
                WHERE (`s`.`payment_provider_order_number` = '?' 
                    OR `s`.`profile_id` = (SELECT `profile_id` FROM `".TBL_FIN_SALE."` WHERE `payment_provider_order_number` = '?')
                ) AND `m`.`membership_id` IS NOT NULL AND `s`.`payment_provider_order_number` != 'admin'
                ORDER BY `s`.`fin_sale_id` DESC LIMIT 1", $subscrId, $subscrId);     
...
function parseCCBILLcardDatalinkResponse( $format, $string )
{
	$_return = array();
...
function get_CCBILLcard_date_format( $time )
{
	return date( "Y", $time ).date( "m", $time ).date( "d", $time ).'010101';
}

function CCBILLcardSaveInLogFile( $string, $filename )
{
	if ( file_exists(DIR_SITE_ROOT.'checkout/CCBILL_card/'.$filename) )
		$mode = (filesize(DIR_SITE_ROOT.'checkout/CCBILL_card/'.$filename)>500000)? "w" : "a";
	else
		$mode = "w";
	
	if ( !$fr = @fopen(DIR_SITE_ROOT.'checkout/CCBILL_card/'.$filename, $mode) )
		return false;
