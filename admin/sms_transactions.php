...
{
	case 'order':
		$sms_payments = SMS_Billing::GetTransactionsByOrder($_REQUEST['order']);
		break;
	case 'profile':
		$sms_payments = SMS_Billing::GetTransactionsByProfile($_REQUEST['profile'], $page, $on_page);
		break;
	default:
		$sms_payments = SMS_Billing::getTransactionsByDate(@$_REQUEST['date_from'], @$_REQUEST['date_to'], $page, $on_page);
		break;
}

$total_num = $sms_payments['total_num'];
...
$frontend->assign('payments', $sms_payments['list']);
$frontend->assign('ord_num', count($sms_payments['list']));
$frontend->assign('total', $sms_payments['total'] );
...
$_page['title'] = 'SMS Payments';

$template = 'sms_transactions.html';
