...
class Thumbnail {
    ...
    **/
    ...
    /**
    *@access public
    *@var string [OPTIONAL] set watermark background text color , RGB Hexadecimal[RECOMENDED ONLY WITH GD 2 ]
    **/
    var $txt_watermark_bg_color = '000000';
    /**
    *@access public
    ...
    *@var integer [OPTIONAL] set watermark text horizonatal margin in pixels
    **/
    var $img_watermark_Hmargin=10;
    /**
    *@access public
    *@var integer [OPTIONAL] set watermark text vertical margin in pixels
    **/
    var $img_watermark_Vmargin=10;
    ...
    **/
    var $memory_limit = app_Image::MEMORY_LIMIT;
    ...
    function process () {
        ...
        //if (false) {
                ...
                if ($this->img_watermark!='' && file_exists($this->img_watermark)) {
                    ...
                    $position_h = ( $this->img_watermark_Haling == 'RIGHT' ) ? $this->calc_position_H() - $this->img_watermark_Hmargin : $this->calc_position_H() + $this->img_watermark_Hmargin;
                    $position_v = ( $this->img_watermark_Valing == 'BOTTOM' ) ? $this->calc_position_V() - $this->img_watermark_Vmargin : $this->calc_position_V() + $this->img_watermark_Vmargin;
                    
                    imagecopyresampled ($this->img["des"], $this->img["watermark"], $position_h, $position_v, 0, 0, $this->img["x_watermark"], $this->img["y_watermark"],$this->img["x_watermark"], $this->img["y_watermark"]);
                }

                if ($this->txt_watermark!='' ) {
                	
                	sscanf($this->txt_watermark_bg_color, "%2x%2x%2x", $red, $green, $blue);
                    $bg_color=imageColorAllocate($this->img["des"] ,$red, $green, $blue);
                     	
                	$text_width = imagefontwidth( $this->txt_watermark_font )*strlen( $this->txt_watermark );
                	$text_height = imagefontheight( $this->txt_watermark_font );
                	
                	imagefilledrectangle( $this->img['des'], $this->calc_text_position_H()-4,$this->calc_text_position_V(), $this->calc_text_position_H()+$text_width+3, $this->calc_text_position_V()+$text_height, $bg_color );
                ...
                if ($this->img_watermark!='' && file_exists($this->img_watermark)) {
                    ...
                    imagecopy ($this->img["des"], $this->img["watermark"], $position_h, $position_v, 0, 0, $this->img["x_watermark"], $this->img["y_watermark"]);
                }
    ...
    function calc_text_position_H () {
        ...
        switch ($this->txt_watermark_Haling) {
             case 'CENTER':
                 ...
                 break;
             case 'RIGHT':
                 $x = $W-$W_mark-($this->txt_watermark_Hmargin)-4;
                 break;
             default:
             case 'LEFT':
                $x = 0+($this->txt_watermark_Hmargin)+4;
                 break;
         }
         return $x;
    }
    ...
    function calc_text_position_V () {
        ...
        switch ($this->txt_watermark_Valing) {
             case 'CENTER':
                 ...
                 break;
             case 'BOTTOM':
                 $y = $H-$H_mark-($this->txt_watermark_Vmargin)-1;
                 break;
             default:
             case 'TOP':
