...
//header('Content-Type: text/plain; charset=ISO-8859-1');
		if ($this->last_result) {
			if ($this->RowCount() > 0) {
				...
				while ($member = mysql_fetch_object($this->last_result)) 
				{
				$json.=	"{\"sender_id  \":\"".$member->sender_id."\",";
				$json.=	"\"message_id \":\"".$member->message_id."\",";
				$json.=	"\"recipient_id \":\"".$member->recipient_id."\",";
				$json.=	"\"conversation_id \":\"".$member->conversation_id."\",";
				$json.=	"\"conversation_number \":\"".$member->conversation_number."\",";
				$json.=	"\"text \": \"".$member->text."\",";
				$json.=	"\"Time \": \"".$member->Time."\",";
				$json.=	"\"sex \": \"".$member->sex."\",";
				$json.=	"\"username \":\"".$member->username."\",";
				$json.=	"\"subject \": \"".$member->subject."\",";
				$json.=	"\"Profile_Pic \":\"".$member->Profile_Pic."\"</br>},";
}
				$json .= '';
				$json = str_replace("},", "},", $json);
				//$json = str_replace("", "", $json);
