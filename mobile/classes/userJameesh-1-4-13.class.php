...
public function usrSignUp($kv)
     {
		...
		$image				=	$_FILES['image']['name'];
                ...
                /*$uname 				=	isset($kv["username"])?$kv["username"]:NULL;
                
		$sex 				=	isset($kv["sex"])?$kv["sex"]:"0";
                
		$match_sex 			=	isset($kv["match_sex"])?$kv["match_sex"]:"0";
		$email				=	isset($kv["email"])?$kv["email"]:NULL;
                
		$password			=	isset($kv["password"])?$kv["password"]:NULL;
                
		$birthdate			=	isset($kv["birthdate"])?$kv["birthdate"]:NULL;
                
		$match_agerange		        =	isset($kv["match_agerange"])?$kv["match_agerange"]:NULL;
                
		$country			=	isset($kv["country_id"])?$kv["country_id"]:NULL;
                
		$state_id			=	isset($kv["state_id"])?$kv["state_id"]:NULL;
               
		$city_id			=	isset($kv["city_id"])?$kv["city_id"]:NULL;
                
		$zip				=	isset($kv["zip"])?$kv["zip"]:NULL;
                
		$headline			=	isset($kv["headline"])?$kv["headline"]:NULL;
                
		$generalDescription	        =	isset($kv["general_description"])?$kv["general_description"]:NULL;
		$join_ip1			=	isset($kv["join_ip"])?$kv["join_ip"]:NULL;
