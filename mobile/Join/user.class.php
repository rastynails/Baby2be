...
mysql_query('SET CHARACTER SET utf8');
      ...
      /*  $phoneNumber 		=	isset($kv["phone_number"])?$kv["phone_number"]:NULL;
                ...
                $uname 				=	trim(isset($kv["username"])?$kv["username"]:NULL,'"');
              
		$sex 				=	trim(isset($kv["sex"])?$kv["sex"]:"0",'"');
            
		$match_sex 			=	trim(isset($kv["match_sex"])?$kv["match_sex"]:"0",'"');
		$email				=	trim(isset($kv["email"])?$kv["email"]:NULL,'"');
		$password			=	trim(isset($kv["password"])?$kv["password"]:NULL,'"');
		$birthdate			=	trim(isset($kv["birthdate"])?$kv["birthdate"]:NULL,'"');
		$match_agerange		        =	trim(isset($kv["match_agerange"])?$kv["match_agerange"]:NULL,'"');
		$country			=	trim(isset($kv["country_id"])?$kv["country_id"]:NULL,'"');
		$state_id			=	trim(isset($kv["state_id"])?$kv["state_id"]:NULL,'"');
		$city_id			=	trim(isset($kv["city_id"])?$kv["city_id"]:NULL,'"');
		$zip				=	trim(isset($kv["zip"])?$kv["zip"]:NULL,'"');
		$headline			=	trim(isset($kv["headline"])?$kv["headline"]:NULL,'"');
		$generalDescription	        =	trim(isset($kv["general_description"])?$kv["general_description"]:NULL,'"');
		...
		$image				=	$_FILES['image']['name'];*/
                ...
                $uname 				=	htmlspecialchars(isset($kv["username"])?$kv["username"]:NULL);
                
		$sex 				=	htmlspecialchars(isset($kv["sex"])?$kv["sex"]:"0");
                
		$match_sex 			=	htmlspecialchars(isset($kv["match_sex"])?$kv["match_sex"]:"0");
		$email				=	htmlspecialchars(isset($kv["email"])?$kv["email"]:NULL);
                
		$password			=	htmlspecialchars(isset($kv["password"])?$kv["password"]:NULL);
                
		$birthdate			=	htmlspecialchars(isset($kv["birthdate"])?$kv["birthdate"]:NULL);
                
		$match_agerange		        =	htmlspecialchars(isset($kv["match_agerange"])?$kv["match_agerange"]:NULL);
                
		$country			=	htmlspecialchars(isset($kv["country_id"])?$kv["country_id"]:NULL);
                
		$state_id			=	htmlspecialchars(isset($kv["state_id"])?$kv["state_id"]:NULL);
               
		$city_id			=	htmlspecialchars(isset($kv["city_id"])?$kv["city_id"]:NULL);
                
		$zip				=	htmlspecialchars(isset($kv["zip"])?$kv["zip"]:NULL);
                
		$headline			=	htmlspecialchars(isset($kv["headline"])?$kv["headline"]:NULL);
                
		$generalDescription	        =	htmlspecialchars(isset($kv["general_description"])?$kv["general_description"]:NULL);
		$join_ip1			=	htmlspecialchars(isset($kv["join_ip"])?$kv["join_ip"]:NULL);
...
$city_id = $sqlChkExe['city_id'];
		...
		// echo "hiii".$image;
			 $orientation=htmlspecialchars(isset($kv['orientation'])?$kv['orientation']:NULL);
        	$keyvalue = array('join_stamp' => time(),'activity_stamp' => time(),'membership_type_id'=>107,'language_id'=>1);
			if($phoneNumber!=NULL)
			{
...
return $value = array('profileid'=>$profileId, 'imgname'=>$image,'orientation'=>$orientation,'sex'=>$sex,'flag'=>0);
	...
	$db5         	=     new MySQL(true, $essence->getDbName(), $essence->getDbHost(), $essence->getDbUser(), $essence->getDbPass());
  	$sqlTypPlan  	="SELECT period,units,is_recurring,price FROM $type_plan_tbl WHERE membership_type_id=107";
  ...
  $sqlFinSale = "INSERT INTO $fin_sale_tbl(profile_id,payment_provider_order_number,order_stamp,unit,period,amount,membership_type_id,status,is_recurring) VALUES($profileId,'trial',$t,'$sqlTPUnits','$sqlTPPeriod','$sqlTPPrice','107','approval','$sqlTPRecurring')";
 ...
 $sqlUserMem = "INSERT INTO $mem_user_tbl (fin_sale_id,start_stamp,expiration_stamp,credits,membership_type_id,prev_membership_type_id,notified) VALUES ($finsale_id,$t,$dateExp,$sqlTPPrice,'107','18','0')";
$db->Query($sqlUserMem); 
	...
	if($db->Query($sql))
	 ...
	 public function SendMailByLimit($id, $skey, $start, $limit) { 
        ...
        if ($res == 1) { 
            ...
            $sql = "SELECT sender_id,message_id,recipient_id,TAB2.conversation_id,TAB2.conversation_number,text,FROM_UNIXTIME(Time,'%c-%d-%Y %H:%i:%s') as Time,sex,username,is_readable,subject,Profile_Pic FROM(SELECT * FROM(SELECT * FROM(SELECT `m`.`message_id`, COUNT(`m`.`message_id`) AS `MailCount`,COUNT(`m`.`conversation_id`) AS `conversation_number`, `m`.`sender_id`, `m`.`recipient_id`, `m`.`is_kiss`, `m`.`text`, `m`.`is_readable`, `m`.`time_stamp` AS `Time`, `c`.`conversation_id` as conv_id, `c`.*, `ms`.`is_replied` FROM `$mail_conv_tbl` AS `c` INNER JOIN ( SELECT * FROM `$mail_msg_tbl` WHERE `sender_id`=$id AND IF (`sender_id`!=$id, `status`='a', 1) ORDER BY `time_stamp` DESC ) AS `m` ON(`m`.`conversation_id`=`c`.`conversation_id`) INNER JOIN ( SELECT `conversation_id`, IF(`sender_id`=$id,'yes','no') AS `is_replied` FROM `$mail_msg_tbl` WHERE (`recipient_id`=$id OR `sender_id`=$id) ORDER BY `time_stamp` DESC ) AS `ms` ON(`ms`.`conversation_id`=`c`.`conversation_id`) WHERE 
...
//echo $sql; 
            //for counting total rows 
            $sqlT = "SELECT sender_id,message_id,recipient_id,TAB2.conversation_id,TAB2.conversation_number,text,FROM_UNIXTIME(Time,'%c-%d-%Y %H:%i:%s') as Time,sex,username,is_readable,subject,Profile_Pic FROM(SELECT * FROM(SELECT * FROM(SELECT `m`.`message_id`, COUNT(`m`.`message_id`) AS `MailCount`,COUNT(`m`.`conversation_id`) AS `conversation_number`, `m`.`sender_id`, `m`.`recipient_id`, `m`.`is_kiss`, `m`.`text`, `m`.`is_readable`, `m`.`time_stamp` AS `Time`, `c`.`conversation_id` as conv_id, `c`.*, `ms`.`is_replied` FROM `$mail_conv_tbl` AS `c` INNER JOIN ( SELECT * FROM `$mail_msg_tbl` WHERE `sender_id`=$id AND IF (`sender_id`!=$id, `status`='a', 1) ORDER BY `time_stamp` DESC ) AS `m` ON(`m`.`conversation_id`=`c`.`conversation_id`) INNER JOIN ( SELECT `conversation_id`, IF(`sender_id`=$id,'yes','no') AS `is_replied` FROM `$mail_msg_tbl` WHERE (`recipient_id`=$id OR `sender_id`=$id) ORDER BY `time_stamp` DESC ) AS `ms` ON(`ms`.`conversation_id`=`c`.`conversation_id`) WHERE 
...
{	
$db3 		= new MySQL(true, $essence->getDbName(), $essence->getDbHost(), $essence->getDbUser(), $essence->getDbPass());				 
$sqlres    =  "Select DISTINCT l.profile_id,username,sex,CONCAT( '/','userfiles/thumb_', CAST( ptbl.profile_id AS CHAR ) , '_',
         CAST( ptbl.photo_id AS CHAR ) , '_',
         CAST( ptbl.index AS CHAR ) , '.jpg' ) as Profile_Pic,latitude,longitude, 
			...
			if($db3->Query($sqlres))
			{
				...
				if($db3->RowCount())
				{
					$stri	=	 '{"count": '.$db3->RowCount().',"result": ['.$db3->GetJSON().']}';
							echo str_replace("},]","}]",$stri);
				}
				else
				{
					echo '{"count": 0,"result": ['.$db3->GetJSON().']}';
				}
			}
			else
			{
